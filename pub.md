...
### Дополнение: совместимость Docker Compose v1/v2
- Исправлена совместимость скриптов установки и обновления с обеими командами:
  - `docker compose` (Compose v2 plugin)
  - `docker-compose` (Compose v1 binary)
- В `install.sh` добавлено автоопределение доступной compose-команды и fallback-установка, если compose отсутствует.
- В `update.sh` добавлено автоопределение compose-команды, чтобы обновление работало одинаково на серверах с v1 и v2.
- Заодно устранена ошибка в `update.sh`: использовался несуществующий helper `error` (заменено на `die`).

Итог: установка/обновление теперь работают на более широком наборе серверов без ручной подгонки под конкретную версию Compose.
